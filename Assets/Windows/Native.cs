...
namespace Interaxon.Libmuse
{
    ...
    internal partial class Native
    {
        ...
        private const int StringBufferDefaultLength = 768;
        ...
        [DllImport(LibmuseDll)]
        private static extern int IxSetProperty(string macAddress, string name, string value, IntPtr errorOut, int errorLen);
        ...
        public static void SetProperty(string macAddress, string name, string value)
        {
            lock (bufferLock)
            {
                if (IxSetProperty(macAddress, name, value, errorBuffer, ErrorBufferLength) != 0)
                {
                    throw ApiError();
                }
            }
        ...
        public static void WriterAddDataPacket(int handle, MuseDataPacket packet, int id = 0)
        {
            lock (bufferLock)
            {
                var values = new double[packet.ValuesSize];
                for (var i = 0; i < packet.ValuesSize; i++)
                {
                    values[i] = packet.Values[i] ?? double.NaN;
                }

                if (IxWriterAddDataPacket(handle, id, packet.PacketType, packet.Timestamp, values, packet.ValuesSize, errorBuffer, ErrorBufferLength) != 0)
                {
                    throw ApiError();
                }
            }
