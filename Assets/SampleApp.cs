...
public class SampleApp : MonoBehaviour
{
    ...
    private void registerAllData()
    {
        ...
        this.muse.listenForDataPacket("OPTICS");
    ...
    private void receiveDataPackets(string data)
    {
        ...
        try
        {
            var json = JsonUtility.FromJson<EEGPacket>(data);
            if (json.values != null && json.values.Length > 0)
            {
                graphManager.AddValue(json.packetType, json.values[0]);
                eegDataStream.AddBandValue(json.packetType, json.values[0]);
                Debug.Log($"Graph Update: {json.packetType} = {json.values[0]}");
            }
        }
        catch (System.Exception e)
        {
            Debug.LogWarning("Failed to parse EEG data: " + e.Message);
        }
    }
