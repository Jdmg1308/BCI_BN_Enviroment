...
public class LibmuseUnityAndroid {
    private static int GLOBAL_REF_MAX = 40000;
    ...
    private final Optics[] opticsMap = new Optics[]{Optics.OPTICS1, Optics.OPTICS2, Optics.OPTICS3, Optics.OPTICS4};
    // Debug string
    ...
    private void receiveMuseDataPacket(final MuseDataPacket p, final Muse muse) {
        if (globalRefCount++ > GLOBAL_REF_MAX) {
            globalRefCount = 0;
    ...
    private void initMaps() {
        ...
        stringToDataPacketType.put("OPTICS", MuseDataPacketType.OPTICS);
        ...
        try{
            switch (p.packetType()) {
                case ACCELEROMETER:
                    ...
                    break;
                case BATTERY:
                    for(int i = 0; i < batteryMap.length; i++) {
                ...
                case PPG:
                    ...
                    break;
                case OPTICS:
                    for(int i = 0; i < opticsMap.length; i++) {
                        result.put(p.getOpticsChannelValue(opticsMap[i]));
                    }
                    break;
