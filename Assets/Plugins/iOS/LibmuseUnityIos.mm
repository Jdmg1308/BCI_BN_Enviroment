...
- (void) connect: (NSString*) headband {
    ...
    if (muse != nil) {
        ...
        // To enable Optics or PPG, see documentation for preset settings of each device type.
        [self.muse setPreset:IXNMusePresetPreset21];
...
- (void) registerDataPacketTypes {
    for(NSString * type in self.dataPacketToListen) {
        ...
        } else if ([type isEqualToString:@"OPTICS"]) {
            [self.muse registerDataListener:self type:IXNMuseDataPacketTypeOptics];
...
- (NSString*) createPacketString:(IXNMuseDataPacketType) type {
    ...
    switch (type) {
        ...
        case IXNMuseDataPacketTypeTemperature:
            packet = @"TEMPERATURE";
            break;
        case IXNMuseDataPacketTypeOptics:
            packet = @"OPTICS";
            break;
...
- (NSString*) createDataString:(IXNMuseDataPacket*) packet {
    ...
    switch (packet.packetType) {
        ...
        case IXNMuseDataPacketTypePpg:
            ...
            break;
        case IXNMuseDataPacketTypeOptics:
            data = [NSString stringWithFormat:@"[%.2f, %.2f, %.2f, %.2f]",
                    [packet getOpticsChannelValue:IXNOpticsOPTICS1],
                    [packet getOpticsChannelValue:IXNOpticsOPTICS2],
                    [packet getOpticsChannelValue:IXNOpticsOPTICS3],
                    [packet getOpticsChannelValue:IXNOpticsOPTICS4]];
            break;
        case IXNMuseDataPacketTypeTemperature:
